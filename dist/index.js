 ...
 */
function discoverTags(version, hash, label, updateStable, alsoLatest, baseTagSize, imageName) {
    ...
    while (versionSchema.length > 1) {
        ...
        if (labelIsBlank || labelIsLts) {
            discoveredTags.push(formatTag(versionSchema.join('.'), '', imageName, versionProps.label));
        }
...
exports.discoverTags = discoverTags;
/**
 ...
 */
function formatTag(version, hash, imageName, label) {
    const hashValue = hash !== '' ? `_${hash}` : '';
    const labelValue = label ? `_${label}` : '';
    const tag = label === SNAPSHOT_LABEL
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core = __importStar(__nccwpck_require__(186));
const discoverer = __importStar(__nccwpck_require__(886));
/**
